...
# Usaremos un archivo JSON para almacenar datos y configuración.
TASKS_FILE = "tasks_data.json"

# --- CONFIGURACIÓN DEL IDIOMA (MÁS ROBUSTA) ---
# Intentamos establecer el locale para que las fechas se muestren en español.
...
# --- MAPAS DE TRADUCCIÓN MANUAL DE SEGURIDAD ---
# Se usan si el setlocale falla y strftime devuelve nombres en inglés.
SPANISH_MONTHS = {
...
def format_date_in_spanish(date_obj):
    """
    Formatea la fecha a español de manera segura (DíaSemana, Día de Mes de Año).
    """
    ...
    # Traducción manual de seguridad si el locale falló
...
CAPTION_STYLE = """
<style>
/* Estilo para los subtítulos de las listas de tareas y calendario, forzando color negro */
    ...
    if os.path.exists(TASKS_FILE):
        try:
            with open(TASKS_FILE, 'r') as f:
                ...
                # Asumiendo que el JSON ahora puede ser solo la lista de tareas si se guarda simplificado
                if isinstance(data, list):
                    tasks = data
                elif isinstance(data, dict) and 'tasks' in data:
                    # Retrocompatibilidad si el formato anterior con 'config' sigue en el archivo
                ...
                for task in tasks:
                    if task.get("date"):
                        # Manejo de casos donde la fecha puede ser None o la cadena ISO
                        if isinstance(task["date"], str):
                            task["date"] = date.fromisoformat(task["date"])
                        elif task["date"] is None:
                            task["date"] = None

                return tasks
        except json.JSONDecodeError:
            st.warning("El archivo de datos está corrupto. Se ha iniciado con una lista vacía.")
            return []
...
def save_tasks():
    ...
    tasks_to_save = []
    # Pre-procesar tareas (date -> string)
    for task in st.session_state.tasks:
        task_copy = task.copy()
        if task_copy.get("date"):
            task_copy["date"] = task_copy["date"].isoformat()
        # Asegurarse de que el 'date' sea None si no tiene fecha, en lugar de un objeto date
        elif "date" in task_copy and task_copy["date"] is None:
             task_copy["date"] = None

        tasks_to_save.append(task_copy)

    # Guardar solo la lista de tareas
...
# La función set_reminder_time ha sido eliminada.

def remove_task(task_id):
    """Elimina una tarea por su ID y guarda los cambios."""
    # El task_id debe ser un string válido (UUID)
    st.session_state.tasks = [task for task in st.session_state.tasks if task["id"] != task_id]
    save_tasks() # Guardar después de eliminar
...
def add_task(description, category_key, due_date):
    ...
    if description:
        ...
        save_tasks() # Guardar después de agregar
    else:
...
with st.sidebar:
    ...
    # Eliminado el bloque de Configuración de Recordatorio
...
# Eliminado el WIDGET DE ESTADO DEL RECORDATORIO
        ...
        for task in tasks:
            ...
            with col_popover:
                ...
                with st.popover("📅 Asignar fecha", use_container_width=True):
                    # Input de fecha, cuyo valor se guarda en el estado de sesión
                    st.date_input(
                    ...
                    # Botón que llama al callback para leer la fecha del input y mover la tarea
                    st.button(
                        "Mover a Calendario",
...
if not tasks_with_date:
    ...
    for task in tasks_with_date:
        # USANDO LA FUNCIÓN DE FORMATO EN ESPAÑOL DE SEGURIDAD
    ...
    for date_str, tasks in tasks_by_date.items():
        # Ordenar tareas dentro del día por nivel de urgencia para destacar la más importante
        tasks.sort(key=lambda t: list(CATEGORIES.keys()).index(t['category_key']))
        top_color = CATEGORIES[tasks[0]['category_key']]['color']

        # Expander (el contenedor del día)
        with st.expander(f"**{date_str}** ({len(tasks)} Tareas)", expanded=True):
            ...
            for task in tasks:
                ...
                with col_desc:
                    # Tarjeta de descripción con barra lateral de color
                    st.markdown(
                        f"""
